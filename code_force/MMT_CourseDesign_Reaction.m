...
for n = 1:m
   ...
   %D,E reaction force
   [Frxd(n),Fryd(n),Frxe(n),Frye(n),Fre(n),Mre(n)] = fRRP2(Dx(n),Dy(n),Ex(n),Ey(n),xs4,ys4,Ex(n)- 125*d,...
    Ey(n) + 25*d,ddxs4,ddys4,Eddx(n),Eddy(n),alpha_DE,0,m4,m5,Js4,0,0,0,-F,0,0);
   
   %D->s3
   [Fcvtx3,Fcvty3,Mcvtf3] = f3PairLinkExternalForce(Dx(n),Dy(n),xs3,ys3,Frxd(n),Fryd(n));
   
   %B,C reaction force
   [Frxb(n),Fryb(n),Frxc(n),Fryc(n),~,~] = fRPR2(Bx(n),By(n),Cx,Cy,Bx(n),By(n),xs3,ys3,...
        Bddx(n),Bddy(n),ddxs3,ddys3,alpha_CD,alpha_CD,theta_CD,m2,m3,Js2,Js3,0,0,0,Fcvtx3,Fcvty3,Mcvtf3,s_B);
